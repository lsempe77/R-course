...
```{r, echo=F}
# This deck uses the intent-to-treat indicator, which is in the IV build
...
m <- df %>%
  filter(round == 0, eligible == 1) %>%
  select(treatment_neighborhood, age_manager, age_deputy,
         educ_manager, educ_deputy, female_manager, foreign_owned,
         staff_size, advanced_filtration, water_treatment_system,
         business_area, recycling_center_distance) %>%
  pivot_longer(-treatment_neighborhood, names_to = "name", values_to = "value") %>%
  group_by(name) %>%
  reframe({
    mod <- estimatr::lm_robust(value ~ treatment_neighborhood)
    tt  <- broom::tidy(mod)
    idx <- tt$term == "treatment_neighborhood"
    tibble::tibble(
      Estimate      = tt$estimate[idx],
      SE            = tt$std.error[idx],
      `P-val`       = tt$p.value[idx],
      `Treat Mean`  = mean(value[treatment_neighborhood == 1], na.rm = TRUE),
      `Control Mean`= mean(value[treatment_neighborhood == 0], na.rm = TRUE)
    )
  }) %>%
  ungroup() %>%
...
m %>%
  select(Variable, `Treat Mean`, `Control Mean`, Estimate, SE, `P-val`) %>%
  kable(
    format = "html",
    booktabs = TRUE,
    caption = "**Diff in Treat and Control Baseline Characteristics**",
...
## Visualizing the RCT Results
```{r, echo=FALSE, fig.height=6.5}
# No covariates

b0 <- coef(out_round1)["(Intercept)"]
b1 <- coef(out_round1)["treatment_neighborhood"]

control <- b0
treated <- b0 + b1
diff_rct <- treated - control

viz_data <- data.frame(
  Time = "Follow-Up",
  Group = c("Control", "Treatment"),
  Expenditure = c(control, treated)
)

# label positions (avoid clipping)
...
ggplot(viz_data, aes(x = Time, y = Expenditure, color = Group)) +
  ...
  labs(
    ...
    color = "",
    caption = sprintf(
      "RCT difference (Treatment - Control): %.2f - %.2f = %.2f",
      treated, control, diff_rct
    )
  ) +
...
# Save
# ggsave("C:/Users/FionaKastel/OneDrive - 3ie/Documents/GitHub/R-course/sessions_in_Abu_Dhabi/case_study_outputs/rct_plot.svg", width = 7, height = 6.5)
```
