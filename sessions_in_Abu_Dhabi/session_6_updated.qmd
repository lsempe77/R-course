...
# m<-df %>%
#   filter(round == 0) %>%
#   select(treatment_neighborhood, age_manager, age_deputy, educ_manager, educ_deputy,
...
modelsummary(list("Round 0"= out_round0, "Round 1"=out_round1), stars = TRUE, coef_map = cm,
...
# Grab the union of terms from both models, in their original order
terms <- unique(unlist(lapply(list(out_round1, out_round2),
                              function(m) modelsummary::get_estimates(m)$term)))

others <- terms[terms != "(Intercept)"]
order  <- append(others, "(Intercept)", after = 1L)  # <- intercept is now 2nd
cm     <- setNames(order, order)

modelsummary(
  list("No covariate adj." = out_round1,
       "With covariate adj." = out_round2),
  coef_map = cm,
  stars = TRUE,
