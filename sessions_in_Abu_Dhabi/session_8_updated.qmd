...
df_treat %>%
  filter(round == 1) %>%
  mutate(enrolled_lab = ifelse(enrolled == 1, "Offered", "Not Offered")) %>%
...
# Basic RDD regression without covariates
out_rdd_no_covs <- lm_robust(waste_management_costs ~ 
...
modelsummary(list("Sharp RDD"= out_rdd_no_covs, "Sharp RDD w Covs"= out_rdd), stars = TRUE,
             gof_map = c("nobs", "r.squared","adj.r.squared"),output = 'kableExtra') %>%
  kable_styling (font_size = 15) 

```

- Treatment reduces waste expenditures by approximately 905 AED
...
- Effect is statistically significant but below the 1,000 AED threshold
