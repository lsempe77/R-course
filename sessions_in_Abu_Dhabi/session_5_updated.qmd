...
- Decision criterion: Program must reduce waste management costs by at least 1,000 AED per year
...
- `enrolled`: Business offered the program (0/1)
...
```{r, eval=T, echo=TRUE}
# Compare waste management costs before and after for enrolled (offered) businesses
...
# Filter df to only run the regression on businesses in treatment neighborhoods AND enrolled
m_ba1 <- lm_robust(waste_management_costs   ~ round, 
                  clusters = neighborhood_identifier,
                  data = df %>% dplyr::filter(treatment_neighborhood ==1 & enrolled ==1))
...
# print(doc, target = "C:/Users/FionaKastel/OneDrive - International Initiative for Impact Evaluation/Documents/GitHub/R-course/sessions_in_Abu_Dhabi/case_study_outputs/session5_before_after.docx")


```  

## Visualizing the Before vs. After Results 

```{r, echo=FALSE, fig.height=6.5}
# Extract coefficients
b0 <- coef(m_ba1)["(Intercept)"]
b1 <- coef(m_ba1)["round"]

# Predicted means at each time (assuming round is 0/1)
before <- b0
after  <- b0 + b1
change <- after - before

ba_viz_data <- data.frame(
  Time = factor(c("Baseline", "Follow-up"), levels = c("Baseline", "Follow-up")),
  Group = "Treated & enrolled",
  Expenditure = c(before, after)
)

ggplot(ba_viz_data, aes(x = Time, y = Expenditure, group = Group)) +
  geom_line(linewidth = 1.2, color = "#00BFC4") +
  geom_point(size = 3, color = "#00BFC4") +
  # vertical difference arrow
  annotate(
  "segment",
  x = 2, xend = 2,
  y = before, yend = after,
  ...
  annotate(
    "text",
    x = 1.5,
    y = min(before, after) - 0.2 * diff(range(c(before, after))),
    label = sprintf("Before–after change:\n%.2f - %.2f = %.2f", after, before, change),
    fontface = "bold"
  ) +
  geom_text(
  aes(x = "Baseline", y = before,
      label = sprintf("%.2f", before)),
  nudge_y = 40
  ) +
  geom_text(
  aes(x = "Follow-up", y = after,
      label = sprintf("%.2f", after)),
  nudge_y = -40
  ) +
  labs(x = "", y = "Waste Expenditures (AED)") +
  theme_minimal() +
  theme(legend.position = "none")

# Save
# ggsave("C:/Users/FionaKastel/OneDrive - 3ie/Documents/GitHub/R-course/sessions_in_Abu_Dhabi/case_study_outputs/before_after_plot.svg", width = 7, height = 6.5)
```
...
```{r, eval=T, echo=TRUE}
# Compare offered (enrolled) vs. non-offered businesses after program implementation
...
#   coef_map = c(
#     "enrolled" = "Enrolled vs. Not-Enrolled Effect",
#     "(Intercept)" = "(Intercept)"
#   ),
#   notes = "Standard errors are in parentheses",
#   fmt = 2,
...
# # doc <- body_add_par(doc, "Before-After Regression Results", style = "heading 1")
# doc <- body_add_flextable(doc, ft)
# print(doc, target = "C:/Users/FionaKastel/OneDrive - International Initiative for Impact Evaluation/Documents/GitHub/R-course/sessions_in_Abu_Dhabi/case_study_outputs/session5_with_without.docx")

```    

## Visualizing the Enrolled vs. Not Enrolled Results 

```{r, echo=FALSE, fig.height=6.5}
# Post-period, treated neighborhoods only
# Extract coefficients
b0 <- coef(m_ba2)["(Intercept)"]
b1 <- coef(m_ba2)["enrolled"]

# Predicted means
not_enrolled <- b0
enrolled_val <- b0 + b1
diff_post <- enrolled_val - not_enrolled

viz_data <- data.frame(
  Time = "Follow-up",
  Group = c("Not enrolled", "Enrolled"),
  Expenditure = c(not_enrolled, enrolled_val)
)

# A small vertical offset that scales with the data range
dy <- 0.03 * diff(range(viz_data$Expenditure))

# Put labels above the higher point and below the lower point
viz_data$label_y <- viz_data$Expenditure + ifelse(
  viz_data$Expenditure == max(viz_data$Expenditure), dy, -dy
)

ggplot(viz_data, aes(x = Time, y = Expenditure, color = Group)) +
  geom_point(size = 3) +

  # numeric labels (computed y position so they never clip)
  geom_text(
    aes(y = label_y, label = sprintf("%.2f", Expenditure)),
    show.legend = FALSE
  ) +
  ...
  annotate(
    "segment",
    x = 1, xend = 1,
    y = min(viz_data$Expenditure),
    yend = max(viz_data$Expenditure),
    arrow = arrow(ends = "both", length = unit(0.1, "inches")),
    linetype = "dashed"
  ) +
  annotate(
    "text",
    x = 1.08,
    y = mean(viz_data$Expenditure),
    label = sprintf("%.2f", diff_post),
    fontface = "bold"
  ) +
  ...
  labs(
    ...
    color = "",
    caption = sprintf(
      "Post-period difference: %.2f - %.2f = %.2f",
      enrolled_val, not_enrolled, diff_post
    )
  ) +

  # Add *top* padding so the top label never gets cut off
  ...
  theme(
    ...
    # keep caption close to plot + bigger
    plot.caption = element_text(
      hjust = 0.5, face = "bold", size = 12,
      margin = margin(t = 4, b = 0)
    ),

    # reduce extra whitespace around legend
    legend.margin = margin(t = 0, b = 0),
    legend.box.margin = margin(t = -4),

    # slightly tighten overall plot margins
    plot.margin = margin(t = 8, r = 5.5, b = 5.5, l = 5.5)
  )


# Save
# ggsave("C:/Users/FionaKastel/OneDrive - 3ie/Documents/GitHub/R-course/sessions_in_Abu_Dhabi/case_study_outputs/enrolled_v_not_plot.svg", width = 7, height = 6.5)
```
...
ft <- modelsummary(list("Model 1: Before-After" = m_ba1, "Model 2: Enrolled vs. Not Enrolled" = m_ba2),
             stars = TRUE,
             gof_map = c("nobs", "r.squared", "r2.adjusted"), 
             title = "Before-After vs. Enrolled-vs-Not-Enrolled Regression Results",
             coef_map = c(
               "round" = "Before vs. After Effect",
               "enrolled" = "Enrolled vs. Not-Enrolled Effect",
               "(Intercept)" = "(Intercept)")
