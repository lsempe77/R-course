...
mean_table <- means %>%
  mutate(enrolled = ifelse(enrolled == 1, "Offered", "Not Offered")) %>%
  pivot_wider(
  names_from = round,
  ...
  arrange(factor(enrolled, levels = c("Offered", "Not offered")))
...
off_row <- mean_table %>% filter(enrolled == "Offered")
ctrl_row <- mean_table %>% filter(enrolled == "Not Offered")
