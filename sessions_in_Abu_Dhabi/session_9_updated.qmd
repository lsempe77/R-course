...
- `enrolled`: Treatment indicator (1=offered, 0=not offered)
...
mean_table <- means %>%
  mutate(enrolled = ifelse(enrolled == 1, "Enrolled", "Not Enrolled")) %>%
  pivot_wider(
  names_from = round,
  values_from = mean_cost,
  names_prefix = "round_"
  ) %>%
  # Reorder columns (after first column)
  select(enrolled, round_1, round_0) %>%
  # Reorder rows so "Offered" comes first
  arrange(factor(enrolled, levels = c("Enrolled", "Not enrolled")))

# Display neatly formatted table
kable(
  mean_table,
  col.names = c("Group", "After (Round 1)", "Before (Round 0)"),
  digits = 2,
...
off_row <- mean_table %>% filter(enrolled == "Enrolled")
ctrl_row <- mean_table %>% filter(enrolled == "Not Enrolled")
...
kable(
  final_table,
  col.names = c("Group", "After (Round 1)", "Before (Round 0)", "Change"),
  digits = 2,
...
#   coef_map = c(
#     "round:enrolled" = "DID Effect",
#     "round" = "Before vs. After",
#     "enrolled" = "Enrolled vs. Not",
#     "(Intercept)" = "(Intercept)"
#   ),
...
# print(doc, target = "C:/Users/FionaKastel/OneDrive - International Initiative for Impact Evaluation/Documents/GitHub/R-course/sessions_in_Abu_Dhabi/case_study_outputs/session9_DID.docx")
...
offered <- c(offered_before, offered_after) 
not_offered <- c(not_offered_before, not_offered_after) 
...
diff_after <- not_offered_after - offered_after
diff_before <- not_offered_before - offered_before
...
ggplot(did_viz_data, aes(x = Time, y = Expenditure, color = Group, group = Group)) +
  geom_line(size = 1.2) +
  geom_point(size = 3) +
  annotate("segment", x = 2, xend = 2, y = offered_after, yend = not_offered_after, 
           arrow = arrow(ends = "both", length = unit(0.1, "inches")), 
           linetype = "dashed") +
  annotate("text", x = 2.1, y = (offered_after + not_offered_after)/2, 
           label = sprintf("%.2f", diff_after)) +
  annotate("segment", x = 1, xend = 1, y = offered_before, yend = not_offered_before, 
           arrow = arrow(ends = "both", length = unit(0.1, "inches")), 
           linetype = "dashed") +
  annotate("text", x = 1.1, y = (offered_before + not_offered_before)/2, 
