...
- **Decision criterion**: Reduce expenditures by at least 1,000 AED
...
methods_df <- data.frame(
  ...
  LowerCI = c(-1092, -1168, -992, -879, -1072),
  UpperCI = c(-935, -797, -818, -753, -927)
)
...
ggplot(methods_df, aes(y = reorder(Method, -abs(Estimate)), x = Estimate)) +
  ...
  labs(title = "Impact of Program Across Methods",
       x = "Impact on Waste Expenditures (AED)",
       y = "") +
  theme_minimal() +
  annotate("text", x = -1000, y = 5.5, label = "Threshold", 
...
synthesis <- data.frame(
  Method = c("Randomized Assignment", "Instrumental Variables", 
             ...
             "Matching (Full)"),
  Estimate = c("-1,014", "-982", "-905", "-816", "-1,000"),
  CI = c("[-1,092, -935]", "[-1,168, -797]", "[-992, -818]", 
         "[-879, -753]", "[-1,072, -927]"),
